...
.venv/bin/pytest backend/tests/test_alignment_engine.py  # single file
.venv/bin/pytest backend/tests/test_fine_cut.py -k "test_name"  # single test

# Lint frontend
cd frontend && npm run lint
```

The Python venv lives at `.venv/` in the project root. Always use `.venv/bin/python` or `.venv/bin/pytest`.
